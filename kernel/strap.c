...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval) {
  ...
  switch (mcause) {
    case CAUSE_STORE_PAGE_FAULT:
      ...
      if(stval<0x7ffff000&&(0x7ffff000-stval)<=20*4*1024)
      {
        void* pa = alloc_page();
        user_vm_map((pagetable_t)current->pagetable, (uint64)stval-stval%PGSIZE, PGSIZE, (uint64)pa,prot_to_type(PROT_WRITE | PROT_READ, 1));
      }
      else
